...
def fit_lstm(train,valid, batch_size, nb_epoch, neurons,dropout,const):
	...
	#model.add(Dropout(0.3))
	#model.add(LSTM(neurons[2], batch_input_shape=(batch_size, X.shape[1], X.shape[2]),kernel_constraint = max_norm(float(const)),recurrent_constraint = max_norm(float(const)),bias_constraint=max_norm(float(const)),dropout=dropout,recurrent_dropout=dropout, stateful=dropout))
	#model.add(Dropout(0.3))
	model.add(Dense(1))
...
def experiment(series,look_back,neurons,n_epoch,dropout,const):
	...
	# fit the model
	lstm_model = fit_lstm(train_scaled,test_scaled, 1, n_epoch, neurons,dropout,const)
...
def run():
	...
	dropout=0.5
	constraint = 4
	experiment(series,look_back,neurons,n_epoch,dropout,constraint)
