...
def fit_lstm(train, batch_size, nb_epoch, neurons,dropout,const):
	...
	#model.add(LSTM(neurons[1], batch_input_shape=(batch_size, X.shape[1], X.shape[2]),kernel_constraint = max_norm(float(const)),recurrent_constraint = max_norm(float(const)),bias_constraint=max_norm(float(const)),dropout=dropout,recurrent_dropout=dropout, stateful=True,return_sequences=True))
	# model.add(Dropout(0.3))
...
def experiment(series, updates,look_back,neurons,n_epoch,dropout,const):
	...
	# fit the model
	lstm_model = fit_lstm(train_scaled, 1, n_epoch, neurons,dropout,const)
...
def run():
	...
	experiment(series, updates,look_back,neurons,n_epochs,dropout,constraint)
