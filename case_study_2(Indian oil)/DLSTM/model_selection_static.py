...
def fit_lstm(X,y, batch_size, nb_epoch, neurons,dropout,const):
	...
	model = Sequential()
	model.add(LSTM(neurons[0], batch_input_shape=(batch_size, X.shape[1], X.shape[2]),kernel_constraint = max_norm(float(const)),recurrent_constraint = max_norm(float(const)),bias_constraint=max_norm(float(const)),dropout=dropout,recurrent_dropout=dropout , stateful=True))
...
def CV(train,scaler,raw_values,epochs,neurons,n_folds,Dropout,constraint):
...
def train_evaluate(ga_individual_solution): 
	...
	#neuron3 = ga_individual_solution[2]
	look_back = ga_individual_solution[2]
	Dropout = ga_individual_solution[3]
	constraint = ga_individual_solution[4]

	print('\nnum of epochs = ',n_epoch,", neurons = [",neuron1,"]","window size = ",look_back,",Dropout = ",Dropout,",constraint = ",constraint)
	...
	# cross validation
	n_folds = 10
	CV_rmse = CV(train_scaled,scaler,raw_values,n_epoch,neurons,n_folds,Dropout,constraint)
...
def genetic():
	...
	toolbox.register("attr_int4",random.randint,1,6)
	toolbox.register("attr_int5",random.choice,np.arange(0.1,1.0,0.1))
	toolbox.register("attr_int6",random.randint,1,4)
	toolbox.register("individual", tools.initCycle, creator.Individual,
             ( toolbox.attr_int2,toolbox.attr_int3,toolbox.attr_int4,toolbox.attr_int5,toolbox.attr_int6),
              n=1)

	toolbox.register('population', tools.initRepeat, list , toolbox.individual)

	toolbox.register('mate', tools.cxTwoPoint)
	toolbox.register('mutate', tools.mutUniformInt,low = [100,1,1,1,1],up = [2000,5,6,9,4],indpb = 0.6)
	...
	for bi in best_individuals:
		...
		best_window = bi[2]
		best_dropout = bi[3]
		best_constraint = bi[4]

		print('\n epochs = ',best_epochs,", neurons = [",best_neuron1,"]","window size = ",best_window,",Dropout = ",best_dropout,",best_constraint = ",best_constraint)
